...
exports.createQuoteWithDetails = async (req, res) => {
  try {
    ...
    await logUserEvent(
      companyId,
      "CREATE",
      `Created a new quote #${quoteData.QuoteNumber} titled "${quoteData.Title}"`,
      {
        newQuote,
      }
    );
...
exports.checkQuoteNumberExists = async (req, res) => {
  try {
    ...
    if (!QuoteNumber) {
      return res
        .status(400)
        .json({ statusCode: 400, message: "QuoteNumber is required" });
    }
    ...
    if (findNumber) {
      await logUserEvent(
        req.user.CompanyId,
        `Quote number ${QuoteNumber} already exists for Company ${CompanyId}`,
        "warning"
      );
      return res
        .status(203)
        .json({ statusCode: 203, message: "Quote number already exists" });
    }

    return res
      .status(200)
      .json({ statusCode: 200, message: "Quote number is available" });
  } catch (error) {
    return res
      .status(500)
...
exports.getCustomerQuotes = async (req, res) => {
  try {
    ...
    if (!CustomerId) {
      return res
        .status(400)
        .json({ statusCode: 400, message: "CustomerId is required!" });
    }
    ...
    const quotes = await Quote.aggregate([
      ...
      {
        $facet: {
          awaitingResponse: [{ $match: { Status: "Awaiting Response" } }],
          changesRequested: [{ $match: { Status: "Request changed" } }],
...
exports.getQuoteDetails = async (req, res) => {
  ...
  if (!response.success) {
    return res
      .status(404)
      .json({ statusCode: 404, message: response.message, data: {} });
  }

  return res
    .status(200)
    .json({ statusCode: 200, message: response.message, data: response.data });
};
  ...
  try {
    ...
    if (!updatedQuote) {
      await logUserEvent(
        req.user.UserId,
        "ERROR",
        `Quote with ID ${QuoteId} not found during update.`
      );
    ...
    await logUserEvent(
      req.user.UserId,
      "ERROR",
      `Error updating quote: ${error.message}`,
      { QuoteId }
    );
  ...
  try {
    ...
    if (updatedQuote || updatedQuoteDetails.modifiedCount > 0) {
      ...
      await logUserEvent(
        req.user.UserId,
        "DELETE",
        `Quote #${quoteNumber} titled '${title}' deleted.`,
        {
          QuoteId: quoteId,
    ...
    await logUserEvent(
      req.user.UserId,
      "ERROR",
      `Error deleting quote with ID ${QuoteId}: ${error.message}`,
      {
        QuoteId,
        DeleteReason,
      }
...
exports.getQuotesByCustomerAndLocation = async (req, res) => {
  try {
    ...
    if (!CompanyId) {
      return res
        .status(400)
        .json({ message: "Unauthorized: CompanyId missing in user data" });
    }
