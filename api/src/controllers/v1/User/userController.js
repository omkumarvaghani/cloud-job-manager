...
exports.updateUser = async (req, res) => {
  try {
    ...
    if (req.user.Role === "Admin" && user.Role !== "Company") {
      return res
        .status(403)
        .json({ message: "Unauthorized: Admin can only update Companies." });
    }
    if (
      req.user.Role === "Company" &&
      !["Worker", "Customer"].includes(user.Role)
    ) {
      return res.status(403).json({
        message: "Unauthorized: Company can only update Workers and Customers.",
      });
    }
...
exports.deleteUser = async (req, res) => {
  try {
    ...
    if (req.user.Role === "Admin" && user.Role !== "Company") {
      return res
        .status(403)
        .json({ message: "Unauthorized: Admin can only delete Companies." });
    }
    if (
      req.user.Role === "Company" &&
      !["Worker", "Customer"].includes(user.Role)
      ...
      return res.status(403).json({
        message: "Unauthorized: Company can only delete Workers and Customers.",
      });
    }
    if (["Worker", "Customer"].includes(req.user.Role)) {
      return res.status(403).json({
