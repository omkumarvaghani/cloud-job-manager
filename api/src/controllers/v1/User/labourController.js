...
exports.createLabour = async (req, res) => {
  try {
    ...
    await logUserEvent(data.CompanyId, "CREATE", "Labour record created", {
      LabourId: userToSave.LabourId,
    });
        ...
        $addFields: {
          Worker: {
            $cond: {
              if: {
                $and: [
                  { $ifNull: ["$WorkerDetails.FirstName", false] },
                  { $ifNull: ["$WorkerDetails.LastName", false] },
                ],
              },
              then: {
                  ...
                  $reduce: {
                    input: {
                      $slice: [
                        ...
                        1,
                        {
                          $size: { $split: ["$WorkerDetails.OwnerName", " "] },
                        },
                      ],
                    },
                    initialValue: "",
...
exports.updateLabour = async (req, res) => {
  try {
    ...
    await logUserEvent(
      req.user.UserId,
      "UPDATE",
      `Labour with ID ${LabourId} for contract ${ContractId} updated.`,
      {
...
exports.deleteLabourData = async (req, res) => {
  try {
    ...
    await logUserEvent(
      req.user.UserId,
      "DELETE",
      `Labour data with ID ${LabourId} deleted.`,
      {
        LabourId,
        ContractId,
