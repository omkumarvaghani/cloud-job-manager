...
exports.getVisitDetails = async (req, res) => {
  try {
          ...
          AssignPersons: {
            $push: {
              $cond: {
                if: {
                  $and: [
                    { $ifNull: ["$ProfileDetails.FirstName", false] },
                    { $ifNull: ["$ProfileDetails.LastName", false] },
                  ],
                },
                then: {
                  $concat: [
    ...
    const Today = result.filter(
      (visit) => new Date(visit.StartDate).getTime() === today.getTime()
    );
    const Upcoming = result.filter(
      (visit) => new Date(visit.StartDate) > today
    );
